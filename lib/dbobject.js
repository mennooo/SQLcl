...
(function(){
  ...
  _addCheck({
    ...
    run: function(options){
      ...
      if (options.uniqueObject.versioning) {
        ...
        var valid = options.connection.util.executeReturnOneCol('select versioncontrol_pkg.check_object_version(p_version_nr => :versionNo, p_object_name => :objectName, p_object_type=> :objectType) from dual', {
          "versionNo": versionNo,
  ...
  _addOutputMethod({
    ...
    setOutput: function(sqlcl2) {
      gByteOutputStream = output.setWebOutput(sqlcl2);
    },
    init: function() {
  ...
  function _fileToDBObject(inputFile) {
    ...
    return {
      ...
      runStatement: function(connection){
        ...
        connection.sqlcl.run();
        connection.sqlcl.setStmt("show errors");
        connection.sqlcl.run();
      }
    };
  };
  ...
  function _setOutputMethod(sqlcl2){
    gOutputMethods[gOutputMethod].run(sqlcl2);
  };
    ...
    if (gErrors.length === 0) {
      ...
      schemas.forEach(function(schema){
        ...
        if (gOutputMethod === output.outputMethods.web){
          result.push(output.getWebOutput(connection.sqlcl, gByteOutputStream));
        }
      ...
      if (gOutputMethod === output.outputMethods.string){
        ...
        application.run('sqldeveloper', gLogFile);
      }
